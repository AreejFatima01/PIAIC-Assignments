//Think of at least three different animals that have a common characteristic.
// Store the names of these animals in a list, and then use a for loop to print out the name of each animal. • Modify your program to print a statement about each animal, such as A dog would make a great pet. • Add a line at the end of your program stating what these animals have in common. You could print a sentence such as Any of these animals would make a great pet!
const animals: string[] = ["Dog", "Dolphin", "Eagle"]; // Modify with your preferred animals
const sharedCharacteristic: string = "intelligence"; // Adjust based on your chosen animals

for (const animal of animals) {
  console.log(
    `${animal.toLowerCase()} is known for its ${sharedCharacteristic}.`
  );
}

console.log(
  `All these animals, despite their differences, share a remarkable level of ${sharedCharacteristic}, making them fascinating creatures!`
);
